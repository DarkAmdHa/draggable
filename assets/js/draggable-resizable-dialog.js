_playDialogButton = document.getElementById("imgDraggable"); //Giant Play Button
var dialog;
function showDialog() {
  if (!dialog) {
    var id = "dialog";
...
function DialogBox(id, callback) {
  var _minW = 1, // The exact value get's calculated
    ...
    _onMouseDown = function (evt) {
      ...
      // we need to take care that this does not mess up normal events
      // on the content of the dialog box, i.e. to copy text
      if (
        !(
          evt.target === _dialog ||
          evt.target === _dialogTitle ||
          evt.target === _buttons[0]
        )
      )
        return;
    ...
    _onMouseMove = function (evt) {
      ...
      if (_isDrag) {
        ...
        if (
          ...
          if (
            !_isButtonHovered ||
            (_isButtonHovered && evt.target != _whichButton)
          ) {
  ...
  (_onMouseUp = function (evt) {
    ...
    (_init = function (id, callback) {
      ...
      _dialog.style.top =
        (window.innerHeight - _dialog.clientHeight) / 2 + "px";
